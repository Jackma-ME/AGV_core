...
void setup()
{
  ...
  nh.advertise(IRsensor_pub);
  nh.advertise(motor_read_pub);
...
void loop()
{
  ...
  {
    updateGoalVelocity();
    motor_driver.controlMotor(WHEEL_SEPARATION, WHEEL_SEPARATION2, goal_velocity);
    tTime[0] = t;
  }
...
void commandVelocityCallback(const geometry_msgs::Twist& cmd_vel_msg)
{
  ...
  goal_velocity_from_cmd[LINEAR_X]  = constrain(goal_velocity_from_cmd[LINEAR_X],  (-1) * MAX_LINEAR_VELOCITY, MAX_LINEAR_VELOCITY);
  goal_velocity_from_cmd[LINEAR_Y]  = constrain(goal_velocity_from_cmd[LINEAR_Y],  (-1) * MAX_LINEAR_VELOCITY, MAX_LINEAR_VELOCITY);
...
void motorPowerCallback(const std_msgs::Bool& power_msg)
{
  ...
  motor_driver.setTorque(DXL_BACK_LEFT_ID, dxl_power);
  motor_driver.setTorque(DXL_BACK_RIGHT_ID, dxl_power);
}
...
{
  static float joint_states_pos[WHEEL_NUM4] = {0.0, 0.0, 0.0, 0.0};
  static float joint_states_vel[WHEEL_NUM4] = {0.0, 0.0, 0.0, 0.0};
  static float joint_states_eff[WHEEL_NUM4] = {0.0, 0.0, 0.0, 0.0};

  if ( goal_velocity[LINEAR_Y] != 0 )
  {
    joint_states_pos[LEFT]  = last_rad[LEFT];
    joint_states_pos[RIGHT] = -last_rad[RIGHT];
    joint_states_pos[BACK_LEFT]  = -last_rad2[LEFT];
    joint_states_pos[BACK_RIGHT] = last_rad2[RIGHT];

    joint_states_vel[LEFT]  = last_velocity[LEFT];
    joint_states_vel[RIGHT] = -last_velocity[RIGHT];
    joint_states_vel[BACK_LEFT]  = -last_velocity2[LEFT];
    joint_states_vel[BACK_RIGHT] = last_velocity2[RIGHT];
  }
...
void updateMotorInfo(int32_t left_tick, int32_t right_tick)
{
  ...
  static int32_t last_tick2[WHEEL_NUM] = {0.0, 0.0};
  ...
  if (init_encoder)
  {
    for (int index = 0; index < WHEEL_NUM; index++)
    {
      ...
      last_tick[index]      = 0.0;
      last_rad[index]       = 0.0;
      last_diff_tick2[index] = 0.0;
      last_tick2[index]      = 0.0;
      last_rad2[index]       = 0.0;

      last_velocity[index]  = 0.0;
      last_velocity2[index]  = 0.0;
    }

    last_tick[LEFT] = left_tick;
    last_tick[RIGHT] = right_tick;
    last_tick2[LEFT]  = left_tick + 0.05;
    last_tick2[RIGHT] = right_tick + 0.05;
  ...
  current_tick = left_tick + 0.05;

  last_diff_tick2[LEFT] = current_tick - last_tick2[LEFT];
  last_tick2[LEFT]      = current_tick;
  last_rad2[LEFT]       += TICK2RAD * (double)last_diff_tick2[LEFT];

  current_tick = right_tick + 0.05;

  last_diff_tick2[RIGHT] = current_tick - last_tick2[RIGHT];
  last_tick2[RIGHT]      = current_tick;
  last_rad2[RIGHT]       += TICK2RAD * (double)last_diff_tick2[RIGHT];
}
...
bool calcOdometry(double diff_time)
{
  ...
  double wheel_l2, wheel_r2;
  double delta_s, delta_y, theta, delta_theta;
  static double last_theta = 0.0;
  double v, w, vy;                  // v = translational velocity [m/s], w = rotational velocity [rad/s]
  double step_time;

  wheel_l = wheel_r = 0.0;
  wheel_l2 = wheel_r2 = 0.0;
  ...
  wheel_l2 = TICK2RAD * (double)last_diff_tick2[LEFT];
  wheel_r2 = -TICK2RAD * (double)last_diff_tick2[RIGHT];
  ...
  if ( goal_velocity[LINEAR_Y] != 0)
  {
    delta_y   = WHEEL_RADIUS * (wheel_l - wheel_r) / 2.0;
  }
  ...
  last_velocity2[LEFT]  = wheel_l2 / step_time;
  last_velocity2[RIGHT] = wheel_r2 / step_time;

  // compute odometric pose
  odom_pose[0] += delta_s * cos(odom_pose[2] + (delta_theta / 2.0)) + delta_y * sin(odom_pose[2] + (delta_theta / 2.0));
  odom_pose[1] += delta_s * sin(odom_pose[2] + (delta_theta / 2.0)) + delta_y * cos(odom_pose[2] + (delta_theta / 2.0));
...
{
  static char *joint_states_name[] = {"wheel_left_joint", "wheel_right_joint", "wheel_back_left_joint", "wheel_back_right_joint"};

  joint_states.header.frame_id = "base_link";
  joint_states.name            = joint_states_name;

  joint_states.name_length     = WHEEL_NUM4;
  joint_states.position_length = WHEEL_NUM4;
  joint_states.velocity_length = WHEEL_NUM4;
  joint_states.effort_length   = WHEEL_NUM4;
}
...
/*******************************************************************************
  Publish Motor Information
*******************************************************************************/
void PublishRead(void)
{
  motor_read_msg.linear.x  = 0;
  motor_read_msg.linear.y  = 0;
  motor_read_msg.linear.z  = 0;
  motor_read_msg.angular.x = 0;

  left_current       = read(portHandler, packetHandler2, 1, 126, 2);
  right_current      = read(portHandler, packetHandler2, 2, 126, 2);
  back_left_current  = read(portHandler, packetHandler2, 3, 126, 2);
  back_right_current = read(portHandler, packetHandler2, 4, 128, 4);

  //motor_current[3] = value1.u32Data;

  motor_read_msg.linear.x  = left_current.u32Data;
  motor_read_msg.linear.y  = right_current.u32Data;
  motor_read_msg.linear.z  = back_left_current.u32Data;
  motor_read_msg.angular.x = back_right_current.u32Data;
  motor_read_pub.publish(&motor_read_msg);
}

/*******************************************************************************
  Motor Information Reading
*******************************************************************************/
...
void IRsensor(void)
{
  ...
  long double voltage_f = 0;
  double distance = 0;
  double distance_f = 0;
  int val = analogRead(sensorPin);
  long double f_val = analogRead(sensorPin);
  //f_val = 0.9752 * f_val + 0.02482 * val;
  f_val = 0.8076 * f_val + 0.1924 * val;

  //  voltage = -3.1802 * pow(10, -9) * pow(val, 3) + 5.7256 * pow(10, -6) * pow(val, 2) + 0.0005 * val + 0.3856;
  //  distance = 19.051 * pow(voltage, 4) - 149.05 * pow(voltage, 3) + 427.62 * pow(voltage, 2) - 555.42 * voltage + 319.33;

  voltage_f = -3.1802 * pow(10, -9) * pow(f_val, 3) + 5.7256 * pow(10, -6) * pow(f_val, 2) + 0.0005 * f_val + 0.3856;  //filtered
  distance_f = 19.051 * pow(voltage_f, 4) - 149.05 * pow(voltage_f, 3) + 427.62 * pow(voltage_f, 2) - 555.42 * voltage_f + 319.33;
